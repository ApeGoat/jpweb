...
const copy = {
    en: { title: "CONTACT", name: "Name", email: "Email", company: "Company", inquiry: "Inquiry type", language: "Language of correspondence", message: "Message", choose: "Select a type", chooseLanguage: "Select a language", types: ["Speaking engagement", "Media", "Collaboration", "Other"], languages: [{ value: "FR", label: "French" }, { value: "EN", label: "English" }], send: "Send message", sending: "Sending…", success: "Thank you. Your message has been sent.", error: "Your message could not be sent. Please try again.", required: "Please complete all required fields." },
    fr: { title: "CONTACT", name: "Nom", email: "Courriel", company: "Entreprise", inquiry: "Type de demande", language: "Langue de correspondance", message: "Message", choose: "Sélectionnez un type", chooseLanguage: "Sélectionnez une langue", types: ["Conférence", "Médias", "Collaboration", "Autre"], languages: [{ value: "FR", label: "Français" }, { value: "EN", label: "Anglais" }], send: "Envoyer le message", sending: "Envoi…", success: "Merci. Votre message a été envoyé.", error: "Votre message n’a pas pu être envoyé. Veuillez réessayer.", required: "Veuillez remplir tous les champs obligatoires." },
};
...
export default function Contact() {
    ...
    const submit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!form.name.trim() || !/^\S+@\S+\.\S+$/.test(form.email) || !form.inquiryType || !form.correspondenceLanguage || !form.message.trim()) { setStatus("invalid"); return; }
        ...
        <main className="content-page">
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.15 }} className="content-panel contact-panel">
                ...
                <form onSubmit={submit} className="contact-form" noValidate>
                    ...
                    <label>{content.language} *<select value={form.correspondenceLanguage} onChange={(e) => update("correspondenceLanguage", e.target.value as "FR" | "EN")} required><option value="">{content.chooseLanguage}</option>{content.languages.map((language) => <option key={language.value} value={language.value}>{language.label}</option>)}</select></label>
